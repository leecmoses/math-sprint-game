...
const updateBestScore = () => {
  bestScoreArray.forEach((score, i) => {
    ...
    if (questionAmount === score.questions) {
      ...
      const savedBestScore = Number.parseFloat(bestScoreArray[i].bestScore);
...
const scoresToDOM = () => {
  finalTimeDisplay = Number.parseFloat(finalTime).toFixed(1);
  baseTime = Number.parseFloat(timePlayed).toFixed(1);
  penaltyTime = Number.parseFloat(penaltyTime).toFixed(1);
...
const getRadioValue = () => {
  ...
  radioInputs.forEach((radioInput) => {
    radioInput.checked ? (radioValue = parseInt(radioInput.value)) : "";
  });
  return radioValue;
};
